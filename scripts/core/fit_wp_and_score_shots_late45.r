...
extract_model_meta <- function(model_obj) {
  ...
  out <- list(
    ...
    uses_late_close_tail = ("late_close_tail" %in% vars) || ("late_time_band" %in% vars),
    uses_late_time_band = "late_time_band" %in% vars,
    uses_late_score_band = any(c("late_score_t0_30", "late_score_t30_45") %in% vars),
    late_tail_variant = attr(model_obj, "late_tail_variant", exact = TRUE),
  ...
  if (length(out$late_tail_time_sec) == 0L || !is.finite(as.numeric(out$late_tail_time_sec))) out$late_tail_time_sec <- 45
  if (length(out$late_tail_score_abs) == 0L || !is.finite(as.numeric(out$late_tail_score_abs))) out$late_tail_score_abs <- 7
  if (length(out$late_tail_variant) == 0L || is.null(out$late_tail_variant) || !nzchar(as.character(out$late_tail_variant))) {
    smooth_labels <- if (!is.null(model_obj$smooth) && length(model_obj$smooth) > 0L) {
      vapply(model_obj$smooth, function(sm) sm$label, character(1))
    } else {
      character()
    }
    out$late_tail_variant <- if ("late_time_band" %in% vars) {
      if (any(c("late_score_t0_30", "late_score_t30_45") %in% vars)) "score-band" else "band"
    } else if ("late_close_tail" %in% vars) {
      if (any(grepl("late_close_tail", smooth_labels))) "surface" else "main"
    } else {
      "none"
    }
  }
...
score_states_with_model <- function(dt_in, model_obj, context = "state rows") {
  ...
  if (isTRUE(meta$uses_late_close_tail) && "late_close_tail" %in% all.vars(formula(model_obj))) {
    pred_dt[, late_close_tail := dt$late_close_tail]
  }
  if (isTRUE(meta$uses_late_time_band)) pred_dt[, late_time_band := dt$late_time_band]
  if (isTRUE(meta$uses_late_score_band)) {
    pred_dt[, late_score_t0_30 := dt$late_score_t0_30]
    pred_dt[, late_score_t30_45 := dt$late_score_t30_45]
  }
...
protocol_m0_formula <- function(use_elo_base = FALSE, late_tail_variant = "none") {
  terms <- c(
    "era",
    "OT_flag",
    "home_possession",
...
protocol_m0_template_meta <- function(
  use_elo_base = FALSE,
  late_tail_variant = "none",
  late_tail_time_sec = 45,
  late_tail_score_abs = 7
) {
  use_late_tail <- !identical(late_tail_variant, "none")
  list(
    ...
    uses_late_close_tail = use_late_tail,
    uses_late_time_band = late_tail_variant %in% c("band", "score-band"),
    uses_late_score_band = identical(late_tail_variant, "score-band"),
    late_tail_variant = late_tail_variant,
...
if (!is.null(model_in)) {
  ...
  wp_formula_template <- protocol_m0_formula(use_protocol_m0_elo_base, late_tail_variant)
...
if (isTRUE(season_oof_predict)) {
  ...
  if (isTRUE(base_meta$uses_late_close_tail)) {
    ...
    cat(sprintf(
      "OOF train uses late-tail variant=%s: time_left_game <= %.1f and abs(score_diff) <= %.1f (rows=%d).\n",
      base_meta$late_tail_variant, base_meta$late_tail_time_sec, base_meta$late_tail_score_abs, sum(dt_oof$late_close_tail == 1, na.rm = TRUE)
    ))
  }
  ...
  oof_worker <- function(i) {
    ...
    if (isTRUE(base_meta$uses_late_close_tail)) {
      attr(fit_oof, "late_tail_time_sec") <- base_meta$late_tail_time_sec
      attr(fit_oof, "late_tail_score_abs") <- base_meta$late_tail_score_abs
      attr(fit_oof, "late_tail_variant") <- base_meta$late_tail_variant
    }
